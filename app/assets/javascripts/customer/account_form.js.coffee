$(document).on 'turbolinks:load', ->
  return if $('div.confirming').length#確認画面では表示非表示をしない
...
toggle_home_address_fields = ->
  checked = $('input#form_inputs_home_address').prop('checked')#booleanを返す(propの引数が1つ)
  $('fieldset#home-address-fields input').prop('disabled', !checked)#値をセットする(propの引数が2つ)
